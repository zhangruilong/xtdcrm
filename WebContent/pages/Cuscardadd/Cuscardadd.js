	...
	var CustomercuscardviewdataForm = new Ext.form.FormPanel({// 定义新增和修改的FormPanel
		...
		, {
			...
			,{
				...
				listeners : {
					blur : function(field, e) {
						Ext.getCmp("cuscardend").setValue(Ext.getCmp("cuscardbegin").getValue()+Ext.getCmp("cuscardday").getValue());
					}
				}
			},{
				xtype : 'textfield',
			...
			handler : function() {
				...
				if (CustomercuscardviewdataForm.form.isValid()) {
					...
					CustomercuscardviewdataForm.form.submit({
						url : basePath + "CuscardService.do?method=addCuscardcustomer",
						waitTitle : '提示',
						waitMsg : '正在处理数据,请稍候...',
						method : 'GET',
						params : {//改

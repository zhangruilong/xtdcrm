...
//			if(CommonConst.SUCCESS.equals(result)) delSolr(temp,AppiontPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//修改
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		for(Appiont temp:cuss){
			if(CommonUtil.isNull(temp.getAppointid())){
				temp.setAppointid(CommonUtil.getNewId());
				result = insSingle(temp);
			}else result = updSingle(temp,AppiontPoco.KEYCOLUMN);
//			if(CommonConst.SUCCESS.equals(result)) updSolr(temp);
		}
		responsePW(response, result);
	}
...
//		Queryinfo queryinfo = getSolrquery(request, Appiont.class, AppiontPoco.QUERYFIELDNAME, AppiontPoco.ORDER, TYPE);

...
//			if(CommonConst.SUCCESS.equals(result)) delSolr(temp,CcustomerPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//修改
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		for(Ccustomer temp:cuss){
			if(CommonUtil.isNull(temp.getCcustomerid())){
				temp.setCcustomerid(CommonUtil.getNewId());
				result = insSingle(temp);
			}else result = updSingle(temp,CcustomerPoco.KEYCOLUMN);
//			if(CommonConst.SUCCESS.equals(result)) updSolr(temp);
		}
		responsePW(response, result);
	}
	...
	public void selLimit(HttpServletRequest request, HttpServletResponse response){
		Queryinfo queryinfo = getQueryinfo(request, Ccustomer.class, CcustomerPoco.QUERYFIELDNAME, CcustomerPoco.ORDER, TYPE, CommonConst.LIMIT);
...
//		Queryinfo queryinfo = getSolrquery(request, Ccustomer.class, CcustomerPoco.QUERYFIELDNAME, CcustomerPoco.ORDER, TYPE);

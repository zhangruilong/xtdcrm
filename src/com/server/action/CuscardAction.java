...
//			if(CommonConst.SUCCESS.equals(result)) delSolr(temp,CuscardPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//修改
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		for(Cuscard temp:cuss){
			if(CommonUtil.isNull(temp.getCuscardid())){
				temp.setCuscardid(CommonUtil.getNewId());
				result = insSingle(temp);
			}else result = updSingle(temp,CuscardPoco.KEYCOLUMN);
//			if(CommonConst.SUCCESS.equals(result)) updSolr(temp);
		}
		responsePW(response, result);
	}
...
//		Queryinfo queryinfo = getSolrquery(request, Cuscard.class, CuscardPoco.QUERYFIELDNAME, CuscardPoco.ORDER, TYPE);

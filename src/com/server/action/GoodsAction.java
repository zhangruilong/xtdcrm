...
//			if(CommonConst.SUCCESS.equals(result)) delSolr(temp,GoodsPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//修改
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		for(Goods temp:cuss){
			if(CommonUtil.isNull(temp.getGoodsid())){
				temp.setGoodsid(CommonUtil.getNewId());
				result = insSingle(temp);
			}else result = updSingle(temp,GoodsPoco.KEYCOLUMN);
//			if(CommonConst.SUCCESS.equals(result)) updSolr(temp);
		}
		responsePW(response, result);
	}
	...
	public void selLimit(HttpServletRequest request, HttpServletResponse response){
		Queryinfo queryinfo = getQueryinfo(request, Goods.class, GoodsPoco.QUERYFIELDNAME, GoodsPoco.ORDER, TYPE, CommonConst.LIMIT);
...
//		Queryinfo queryinfo = getSolrquery(request, Goods.class, GoodsPoco.QUERYFIELDNAME, GoodsPoco.ORDER, TYPE);

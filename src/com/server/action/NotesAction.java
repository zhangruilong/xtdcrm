...
//			if(CommonConst.SUCCESS.equals(result)) delSolr(temp,NotesPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//修改
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		for(Notes temp:cuss){
			if(CommonUtil.isNull(temp.getNotesid())){
				temp.setNotesid(CommonUtil.getNewId());
				result = insSingle(temp);
			}else result = updSingle(temp,NotesPoco.KEYCOLUMN);
//			if(CommonConst.SUCCESS.equals(result)) updSolr(temp);
		}
		responsePW(response, result);
	}
...
//		Queryinfo queryinfo = getSolrquery(request, Notes.class, NotesPoco.QUERYFIELDNAME, NotesPoco.ORDER, TYPE);

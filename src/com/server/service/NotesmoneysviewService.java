package com.server.service;

import java.lang.reflect.Type;
import com.google.gson.reflect.TypeToken;
import java.util.ArrayList;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
//import org.apache.solr.common.SolrDocumentList;
import com.server.pojo.Notesmoneysview;
import com.server.poco.NotesmoneysviewPoco;
import com.server.action.NotesmoneysviewAction;
import com.system.tools.CommonConst;
import com.system.tools.base.BaseActionDao;
import com.system.tools.pojo.Fileinfo;
import com.system.tools.pojo.Queryinfo;
import com.system.tools.util.CommonUtil;
import com.system.tools.util.FileUtil;
import com.system.tools.util.TypeUtil;
import com.system.tools.pojo.Pageinfo;

/**
 * notesmoneysview 服务层
 *@author ZhangRuiLong
 */
public class NotesmoneysviewService extends NotesmoneysviewAction {
	//查询参与活动的人员
	public void selhuodong(HttpServletRequest request, HttpServletResponse response){
		String huodong = request.getParameter("huodong");
		Queryinfo queryinfo = getQueryinfo(request, Notesmoneysview.class, NotesmoneysviewPoco.QUERYFIELDNAME, NotesmoneysviewPoco.ORDER, TYPE);
		queryinfo.setWheresql("notesdetail like '%活动"+huodong+"%'");
		Pageinfo pageinfo = new Pageinfo(getTotal(queryinfo), selQuery(queryinfo));
		result = CommonConst.GSON.toJson(pageinfo);
		responsePW(response, result);
	}
}
